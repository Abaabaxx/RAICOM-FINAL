...
# 最新增：添加外部控制的启停机制，默认关闭状态
# 【最新】：添加"指令保鲜"策略，使用灵活的循环版本替代硬编码重复发布
"""
...
8.  **【新增】灵活指令保鲜策略**: 使用可配置的循环发布机制，避免控制真空期导致的急刹问题。
...
python /home/lby/tianbot_ws/src/f1/scripts/ftg_node.py
...
# 启动FTG节点
python /home/lby/tianbot_ws/src/f1/scripts/controller.py
...
class FollowTheGapNode:
    def __init__(self):
        ...
        # --- 【新增】用于"指令保鲜"的变量 ---
        self.last_drive_command = AckermannDriveStamped() # 存储最近一次发布的有效指令
        self.last_drive_command.drive.speed = 0.0
        self.last_drive_command.drive.steering_angle = 0.0
        rospy.loginfo("FTG Node: Initialized command keep-alive system.")
    ...
    # --- 【修改】导航模式的回调函数 - 使用灵活的循环版本替代硬编码重复发布 ---
    def mode_callback(self, msg):
        """
        接收来自导航管理器的模式指令，并更新当前状态。
        【新增】在切换到TEB时，执行灵活的"指令保鲜"操作，避免控制真空期。
        """
        new_mode = msg.data
        
        # 只有当模式真正发生改变时才执行逻辑
        if self.current_mode != new_mode:
            rospy.loginfo("FTG_Node: Received new mode command: {}. Previous mode was {}.".format(new_mode, self.current_mode))

            # --- 【核心逻辑】灵活的指令保鲜策略 ---
            # 检查是否是从FTG模式切换到TEB模式
            if new_mode == "TEB_MODE" and self.current_mode == "FTG_MODE":
                rospy.loginfo("FTG_Node: Transitioning from FTG to TEB. Executing flexible 'command keep-alive' strategy.")
                
                # 创建一个修改版的"保鲜"指令：保持速度，但方向盘回正
                # 这样可以防止车辆在TEB接管前继续转向，确保平滑过渡
                fresh_command = AckermannDriveStamped()
                fresh_command.header.frame_id = "base_link"
                fresh_command.drive.speed = self.last_drive_command.drive.speed  # 保持最后的速度
                fresh_command.drive.steering_angle = 0.0  # 方向盘回正，防止异常转向
                
                # --- 【可配置的参数】指令保鲜"旋钮" ---
                PUBLISH_COUNT = 5        # 【可调】你想发布的总次数 (推荐2-5次)
                PUBLISH_INTERVAL = 0.033 # 【可调】每次发布之间的间隔（秒，0.033≈30Hz）
                # -----------------------------------------------------
                
                rospy.loginfo("FTG_Node: Publishing {} keep-alive commands at {:.0f} Hz interval (speed={:.2f}, steering=0.0)".format(
                    PUBLISH_COUNT, 1.0/PUBLISH_INTERVAL, fresh_command.drive.speed))
                
                # 循环发布指定次数的保鲜指令
                for i in range(PUBLISH_COUNT):
                    # 每次发布前都更新时间戳，确保指令是"新鲜"的
                    fresh_command.header.stamp = rospy.Time.now()
                    self.drive_pub.publish(fresh_command)
                    
                    # 在最后一次发布后，不执行延时
                    if i < PUBLISH_COUNT - 1:
                        rospy.sleep(PUBLISH_INTERVAL)
                
                rospy.loginfo("FTG_Node: All keep-alive commands sent successfully. FTG is now fully paused, waiting for TEB takeover.")

            # 最后，更新当前模式状态
            self.current_mode = new_mode

        else:
            rospy.logdebug("FTG_Node: Received redundant mode command: {}".format(new_mode))
    ...
    # --- 【修改】发布控制指令函数 - 添加指令保存逻辑 ---
    def publish_control_command(self, speed, steering_angle):
        """
        七、创建并发布阿克曼驱动消息，并保存为"最后一帧指令"用于指令保鲜。
        ...
        # --- 【新增】保存这一帧指令用于"指令保鲜" ---
        self.last_drive_command = drive_stamped_msg
        rospy.logdebug("FTG_Node: Command saved for keep-alive: speed={:.2f}, steering={:.3f}".format(
    ...
    def process_lidar(self, data):
        ...
        # 9. 发布控制指令（现在会自动保存到last_drive_command）
        self.publish_control_command(speed, final_steering_angle)

    # --- 【简化】雷达回调函数 - 移除复杂的模式切换处理逻辑 ---
    ...
    def run(self):
        ...
        rospy.loginfo("【ENHANCED FEATURE】Flexible command keep-alive system activated for smooth mode transitions.")
        rospy.loginfo("【CONFIG TIP】Adjust PUBLISH_COUNT and PUBLISH_INTERVAL in mode_callback for fine-tuning.")
