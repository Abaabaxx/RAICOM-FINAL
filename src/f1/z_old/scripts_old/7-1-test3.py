...
# 自动同步RVIZ和Gazebo的状态（该版本测试后废弃，没有用）
# 为Follow The Gap算法添加RViz可视化功能，显示滤波和限距后的雷达点、安全气泡区域、识别出的最大间隙、最近的障碍物点（作为标记）、最佳目标点（作为标记）。
"""
...
rosrun rviz rviz -d /home/lby/tianbot_ws/src/f1/rviz/nav.rviz
rosrun rviz rviz -d /home/lby/tianbot_ws/src/f1/rviz/nav.rviz
...
class FollowTheGapNode:
    def __init__(self):
        ...
        # 新增：用于检测时间重置的状态变量
        self.last_time = rospy.Time.from_sec(0.0)
        ...
        rospy.loginfo("Follow The Gap Node with dynamic reconfigure and visualization has been initialized.")

    def clock_callback(self, msg):
        """
        监听/clock话题，检测仿真时间是否回跳（意味着Gazebo重置）。
        
        参数:
            msg: rosgraph_msgs/Clock消息
        """
        # 如果当前时间小于上次记录的时间，说明发生了重置
        if msg.clock < self.last_time:
            rospy.logwarn("Gazebo reset detected! Clearing visualizations.")
    ...
    def clear_visualizations(self):
        """
        发布空消息或删除消息来清空RViz中的所有可视化内容。
        """
        # 1. 清空所有标记
        clear_marker_array = MarkerArray()
        clear_marker = Marker()
        clear_marker.ns = "ftg_markers"  # 需要与你发布的标记有相同的namespace
        clear_marker.action = Marker.DELETEALL  # 使用 DELETEALL 动作
        clear_marker_array.markers.append(clear_marker)
        self.marker_pub.publish(clear_marker_array)

        # 2. 清空点云 (通过发布一个空点云)
        # 创建一个通用的header
        header = Header()
        header.stamp = rospy.Time.now()
        header.frame_id = "base_link"  # 或者 "tianracer/base_link"

        # 创建一个空点云
        empty_cloud = pc2.create_cloud_xyz32(header, [])
        
        # 发布空点云到所有点云话题
        self.processed_points_pub.publish(empty_cloud)
        self.bubble_pub.publish(empty_cloud)
        self.gap_pub.publish(empty_cloud)
        
        rospy.loginfo("All visualizations cleared due to Gazebo reset.")
    ...
    def process_lidar(self, data):
        """
        雷达寻隙算法(Follow The Gap)的主体实现，包含可视化功能
        ...
        if self.reset_detected:
            ...
            self.reset_detected = False  # 重置标志位，以免每次都清空
            return  # 本次回调不进行任何处理，等待下一次数据
        ...
        # 6. 计算转向角
        steering_angle = self.get_angle(best_point_index, len(proc_ranges))
        
        # 7. 根据转向角与直行和拐角分界的角度阈值判断当前是直行还是拐角状态,调整速度
    ...
    def run(self):
        ...
        保持节点运行
        """
        rospy.loginfo("Follow The Gap Node with visualization is running. Use rqt_reconfigure to adjust parameters.")
