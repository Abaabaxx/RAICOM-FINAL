...
if __name__ == '__main__': 
    try:
        # 修改话题名称，添加 tianracer 命名空间前缀
        odom_sub = rospy.Subscriber('/tianracer/odom', Odometry, xwcpos_callback)
        scan_sub = rospy.Subscriber('/tianracer/scan', LaserScan, xwcgap_callback)  # 修改为带有命名空间的话题
        drive_pub = rospy.Publisher('/tianracer/ackermann_cmd_stamped', AckermannDriveStamped, queue_size=1)  # 保持原有的命名空间
        rospy.spin()
